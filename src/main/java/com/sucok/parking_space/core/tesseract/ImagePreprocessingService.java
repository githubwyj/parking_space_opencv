...
public class ImagePreprocessingService {
    ...
    public ImagePreprocessingService() {
        this.net = Dnn.readNetFromTensorflow(ImageUtil.readFileFromClasspath("east/frozen_east_text_detection.pb").getAbsolutePath());
    }
    ...
    private Observable<Mat> convert(File image) {
        ...
        byte[] imageArray = ImageUtil.file2Byte(image);
        ...
        return Observable.create(new ObservableOnSubscribe<Mat>() {
            ...
            public void subscribe(ObservableEmitter<Mat> emitter) throws Exception {
                ...
                //裁剪为原来尺寸
                org.bytedeco.opencv.opencv_core.Rect rect = new org.bytedeco.opencv.opencv_core.Rect(60, 140, 150, 360);
                Mat  newImg = ImageUtil.matCovert(ImageUtil.cutImage(ImageUtil.matCovert(imageMat),rect));

                emitter.onNext(newImg);
            }
        });
    }

    private Observable<Mat> rotate(Mat imageMat) {

        return Observable.create(new ObservableOnSubscribe<Mat>() {
            @Override
            public void subscribe(ObservableEmitter<Mat> emitter) throws Exception {
                emitter.onNext(ImageUtil.imageRotate(imageMat, ANGLE, 1));
            }
        });
    }
